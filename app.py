...
# Funciones auxiliares (mantenemos igual)
...
def obtener_estado(prop):
    try:
        # Simplificamos los estados: Leído, Leyendo, Pendiente
...
try:
    ...
    with st.spinner('Cargando estantería...'):
        respuesta = requests.post(url, headers=headers, json={"page_size": 100})
        ...
        def formato_mensaje(rec):
            ...
            if rec['saga_texto']:
                num = rec['numero_saga']
                num_texto = f" (Libro {int(float(num))})" if num else ""
                mensaje += f" \n*Pertenece a {rec['saga_texto']}{num_texto}*"
            return mensaje

        col1, col2, col3 = st.columns(3)
        
        # Lógica de botones... (mantenemos la lógica anterior)
        with col1:
            if st.button("🔮 Mismo género", use_container_width=True):
                g_ref = set(ref["generos"])
                a_ref = set(ref["autores_ids"])
                opciones = [p for p in pendientes if not set(p["autores_ids"]).intersection(a_ref) and len(g_ref.intersection(set(p["generos"]))) >= 1]
        ...
        with col2:
            if st.button("🔄 Cambio radical", use_container_width=True):
                opciones = [p for p in pendientes if not any(g in p["generos"] for g in ref["generos"])]
        ...
        with col3:
            if st.button("🎨 Buscar por Color", use_container_width=True):
                ...
                opciones = [p for p in pendientes if set(p.get("colores", [])).intersection(c_ref)]
